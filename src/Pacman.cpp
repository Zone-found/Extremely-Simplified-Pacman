...
int window_id; //创建窗体时的返回值
bool replay = false; //检查是否启动游戏
bool menu = true; //检查是否处于菜单界面
bool over = true; //检查游戏是否结束
float squareSize = 50.0; //一个单元大小
float xIncrement = 0; // x坐标
float yIncrement = 0; // y坐标
int rotation = 0; // 定位
float* monster1 = new float[3] {10.5, 8.5, 1.0}; //第一个小鬼怪的坐标和方向
float* monster2 = new float[3] {13.5, 1.5, 2.0}; //第二个小鬼怪的坐标和方向
float* monster3 = new float[3] {4.5, 6.5, 3.0}; //第三个小鬼怪的坐标和方向
float* monster4 = new float[3] {2.5, 13.5, 4.0}; //第四个小鬼怪的坐标和方向
...
int main(int argc, char** argv) {
	...
	glutInitWindowSize(750, 600);//窗口大小
	glutInitWindowPosition(265, 25);//窗口起始位置
	window_id = glutCreateWindow("Extremely Simplified Pacman - by Zone & Little Cutie & Aglaia");
