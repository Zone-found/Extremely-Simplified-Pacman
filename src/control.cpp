...
void keyPressed(unsigned char key, int x, int y) {
	...
	if (!replay && over) {
		if (key == 'w') {
			if (start_menu != NULL) {
				int selected_index = start_menu->getSelectedIndex();
				start_menu->selectButton(selected_index - 1);
			}
		}
		if (key == 's') {
			if (start_menu != NULL) {
		...
		if (key == ' ') {
			if (start_menu != NULL) {
				start_menu->callButtonFunc(start_menu->getSelectedIndex());
			}
		}
	}
	else if (replay && over && key == '\r') {
		replay = false;
	}
}
